...
def main(
    ...
    if training_args.do_eval:
        ...
        if additional_args.plotting_logits:
            ...
            padded_data = [
                np.pad(
                    np.array(arr, dtype=float),  # Convert array to float
            ...
            plt.grid(True)
            plt.savefig(
                "plots/boxplot_top1_rank_eval"
                + data_args.dataset_name.replace("/", "_")
                + "_"
