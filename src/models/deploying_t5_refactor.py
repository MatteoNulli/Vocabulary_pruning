...
class DeployT5Attention(T5Attention):
    def __init__(self, config: T5Config, has_relative_attention_bias=False):
        ...
        self.is_decoder = (
            config.is_decoder
        )  # Boolean indicating if this is a decoder module
        self.has_relative_attention_bias = (
            has_relative_attention_bias  # Boolean for relative attention bias usage
        )
        ...
        )  # Compute the inner dimension for the model

        ####### Mesh TensorFlow initialization to avoid scaling before softmax #######
        self.q = nn.Linear(
            self.d_model, self.inner_dim, bias=False
        ...
        )  # Linear transformation to output from attention heads
    ...
    def forward(
        ...
        if past_key_value is not None:
            ...
            if query_length is None:
                ...
                if stack_hidden_states is not None:
                    real_seq_length += stack_hidden_states.shape[1]
            else:
        ...
        if (
            ...
            and stack_hidden_states is not None
        ):
        ...
        if self.config.use_synchronize:
            torch.cuda.synchronize()
        if self.is_decoder:
            self.key_value_gen_time = datetime.datetime.now() - start
        ...
        if skip_mask:
            ...
            query_states = rearrange(
                self.q(hidden_states), "b l (h d) -> b h l d", h=self.n_heads
            )
        ...
        if self.config.use_synchronize:
            torch.cuda.synchronize()
        if self.is_decoder:
            self.attn_ffn_time = datetime.datetime.now() - start
...
class DeployT5LayerSelfAttention(T5LayerSelfAttention):
    def __init__(self, config, has_relative_attention_bias=False):
        super().__init__(config, has_relative_attention_bias)
        self.config = config
        self.SelfAttention = DeployT5Attention(
    ...
    def forward(
        ...
        normed_stack_hidden_states = (
            self.layer_norm(stack_hidden_states)
            if stack_hidden_states is not None
            else None
        )
        if self.config.use_synchronize:
        ...
        if self.config.is_decoder:
            self.attn_ffn_time = (
                self.SelfAttention.attn_ffn_time
                + norm_time
...
class DeployT5LayerCrossAttention(T5LayerCrossAttention):
    def __init__(self, config):
        ...
        self.config = config
        self.EncDecAttention = DeployT5Attention(
    ...
    def forward(
        ...
        if self.config.is_decoder:
            self.attn_ffn_time = (
                self.EncDecAttention.attn_ffn_time
                + norm_time
                + (datetime.datetime.now() - start)
            )
            self.key_value_gen_time = self.EncDecAttention.key_value_gen_time
...
class DeployT5Block(T5Block):
    def __init__(self, config, has_relative_attention_bias=False):
        ...
        self.is_decoder = (
            config.is_decoder
        )  # Flag to check if this block is part of the decoder

        # Initialize layers in the block
        self.layer = nn.ModuleList()
        self.layer.append(
            DeployT5LayerSelfAttention(
                config, has_relative_attention_bias=has_relative_attention_bias
            )
        )
    ...
    def forward(
        ...
        if self.is_decoder:
            ...
            self.key_value_gen_time = (
                self.layer[0].key_value_gen_time,
                self.layer[1].key_value_gen_time,
            )
...
class DeployT5Stack(T5Stack):
    def __init__(self, config, embed_tokens=None):
        ...
        self.block = nn.ModuleList(
            [
                DeployT5Block(config, has_relative_attention_bias=bool(i == 0))
        ...
        self.block_op = [
            0
        ] * config.num_layers  # to calculate the average number of forward block layers
    ...
    def parallel_gen_token(
        ...
        for j in range(layer_idx, len(self.block)):
            ...
            if past_key_value is None:
                ...
                self.deploy_time["time_parallel_key_value_gen"][1] += self.block[
                    j
                ].key_value_gen_time
            ...
            if extended_attention_mask is None or position_bias is None:
                ...
                if self.config.parallel_causal_mask and extended_attention_mask is None:
                    ...
                    extended_attention_mask = self.get_extended_attention_mask(
                        attention_mask,
                        torch.Size([hidden_states.shape[0], hidden_states.shape[1]]),
                    )
            ...
            layer_outputs = self.block[
                j
            ](  #### Block forward pass, should be outputting the logits indeed no?
                hidden_states,
                attention_mask=extended_attention_mask,
                position_bias=position_bias,
                encoder_hidden_states=encoder_hidden_states,
            ...
            for idx, t in enumerate(self.block[j].key_value_gen_time):
                self.deploy_time["time_parallel_key_value_gen"][idx] += t
            for idx, t in enumerate(self.block[j].attn_time):
                self.deploy_time["time_parallel_attn"][idx] += t
    ...
    def forward(
        ...
        for i, layer_module in enumerate(self.block):
            ...
            if self.is_decoder:
                ...
                for idx, t in enumerate(layer_module.key_value_gen_time):
                    self.deploy_time[prefix + "key_value_gen"][idx] += t
                for idx, t in enumerate(layer_module.attn_time):
                    self.deploy_time[prefix + "attn"][idx] += t
        ...
        ):  # If threshold is "not satisfied", then compute the new block logits
...
class DeployT5ForConditionalGeneration(T5ForConditionalGeneration):
    def __init__(self, config):
        ...
        if (
            self.config.exit_conf_type == "meta"
            or self.config.shallow2deep_conf_type == "meta"
        ):
    ...
    def forward(
        ...
        encoder_outputs, decoder_outputs = self.forward_impl(
            input_ids,
            attention_mask,
            decoder_input_ids,
        ...
        if self.decoder.lm_logits is None:  # token has not skipped
            ...
            lm_logits = self.decoder.lm_logits
    ...
    def forward_impl(
        ...
        if (
            ...
            and len(self.decoder.stack_conf_all) > 0
            and self.bmm_update_iter < self.bmm_update_max_iter
        ):
        ...
        for k, v in self.decoder.deploy_time.items():
            if type(v) != list:
                self.deploy_time[k] += v
            else:
                self.deploy_time[k] = [
                    _d + _v for _d, _v in zip(self.deploy_time[k], v)
                ]
    ...
    def greedy_search(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            # Not done in the case of our experiments.
